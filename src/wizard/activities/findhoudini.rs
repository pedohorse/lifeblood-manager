...
pub struct FindHoudiniActivity {
    widgets: Option<
        Rc<
            RefCell<
                Vec<(
        ...
        >,
    >,
    init_data: Vec<(PathBuf, (u32, u32, u32), (u32, u32))>,
}

impl WizardActivityTrait for FindHoudiniActivity {
    fn start_activity(&mut self) {
        ...
        Frame::default()
            ...
            .with_label(
                "\
        Now help me locating Houdini executables.\n\
        Provide me with the paths to the directory where houdini executable is located  for each\n\
        version of Houdini you want to use.\n\
        \n\
        Also specify the correct version number of each Houdini, and python version that houdini uses.\n\
        Houdini submitter node will rely on these version numbers being correct.\
        ",
            );
        layout.end();
        ...
        for i in 0..MAX_VER_NUM {
            ...
            let py_label = Frame::default().with_label("python:");
            let mut py_maj = IntInput::default();
            py_maj.set_value("3");
            let py_min = IntInput::default();
            ...
            row_layout.fixed(&py_label, 56);
            row_layout.fixed(&py_maj, 32);
            row_layout.fixed(&py_min, 32);
            row_layout.end();
            ...
            user_inputs.push((
                row_layout, bin_path, ver_maj, ver_min, ver_patch, py_maj, py_min,
            ));
        }
        main_layout.end();
        ...
        for (i, ver_data) in self.init_data.iter().enumerate() {
            ...
            input_data.5.set_value(&ver_data.2 .0.to_string());
            input_data.6.set_value(&ver_data.2 .1.to_string());
        }
        ...
        if let Some(ref widget_tuples) = self.widgets {
            for widget_tuple in widget_tuples.borrow().iter() {
                ...
                if !bin_path.is_absolute() {
                    return Err("provided path to houdini bin is not an absolute path");
                }
                if !bin_path.exists() {
                    return Err("provided path to houdini bin does not exist");
                }
                if !bin_path
                    .join(if cfg!(windows) {
                        "houdini.exe"
                    ...
                    .exists()
                {
                    return Err("provided path does not contain houdini executable");
                }
                // validate version numbers
                macro_rules! version_check {
                    ($ver:expr) => {
                        if let Err(_) = u32::from_str_radix(&$ver, 10) {
                            return Err("houdini version components must be set");
                        }
                    };
                }
                ...
                version_check!(widget_tuple.5.value());
                version_check!(widget_tuple.6.value());
            }
        }
        Ok(())
    }
...
impl FindHoudiniActivity {
    pub fn new(init_data: Vec<(PathBuf, (u32, u32, u32), (u32, u32))>) -> Self {
        FindHoudiniActivity {
            widgets: None,
            init_data: init_data,
        }
    ...
    pub fn selected_versions(&self) -> Option<Vec<(PathBuf, (u32, u32, u32), (u32, u32))>> {
        if let Some(ref widgets_ref) = self.widgets {
            ...
            for widgets in widgets_ref.borrow().iter() {
                ...
                ret.push((
                    ...
                    (
                        u32::from_str_radix(&widgets.5.value(), 10).unwrap_or(0),
                        u32::from_str_radix(&widgets.6.value(), 10).unwrap_or(0),
                    ),
                ));
            }
            Some(ret)
